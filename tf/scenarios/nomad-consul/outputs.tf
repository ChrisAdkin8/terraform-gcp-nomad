output "cluster_summary" {
  value = join("\n", [
    ...
    "Alloy Collector UI Endpoint : http://collector.traefik-dc1.${var.project_id}.${local.base_domain}:8080",
    "Alloy Gateway Loki Endpoint : http://gateway-api.traefik-dc1.${var.project_id}.${local.base_domain}:8080/loki/api/v1/push",
    "Alloy Gateway UI Endpoint   : http://gateway-ui.traefik-dc1.${var.project_id}.${local.base_domain}:8080",
    "Loki Endpoint               : http://loki.traefik-dc1.${var.project_id}.${local.base_domain}:8080",
    "Grafana Management Console  : http://grafana.traefik-dc1.${var.project_id}.${local.base_domain}:8080",
    "",
    ...
    "",
    "### Secondary Cluster Endpoints & Details ###",
    "---------------------------------------------",
